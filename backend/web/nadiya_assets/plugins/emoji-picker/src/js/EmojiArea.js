...
export default class EmojiArea {
  constructor(emojiArea, options) {
    ...
    $('.chat-input').on('keyup paste focus click', function(){
      if (window.getSelection) {
        $this.saveSelection()
            var sel = window.getSelection();
                if($(sel.anchorNode).closest('.chat-input').length){
                    $this.anchorNode = {
    ...
    if (options.type !== 'unicode') {
      ...
      const v = this.$ti.val();
      if('setSelectionRange' in this.$ti[0])
  ...
  saveSelection() {
    ...
    if (this.$e === this.$ti && e.selectionStart && e.selectionEnd) {
      ...
      if (sel.focusNode && (sel.focusNode === e || sel.focusNode.parentNode === e || sel.focusNode.parentNode.parentNode === e)){
  ...
  insert(alias) {
    ...
    if (!this.replaceSelection(content)) {
        ...
        if(this.anchorNode){
            anchor = this.anchorNode.anchorNode;
            offset = this.anchorNode.offset+content.length;
            // if(!$(el).html()){
            //     $(el).trigger('click');
            // }
            if(anchor.data){
                anchor.data = anchor.data.slice(0,this.anchorNode.offset) + content + anchor.data.slice(this.anchorNode.offset,anchor.data.length);
            }
            else{
              $(anchor).find('br').remove();
                $(anchor).closest('div').html($(anchor).closest('div').html()+content);
                anchor = $(anchor)[0].childNodes[0];
            }
        }
        else{
            if($(el).children().last().children().last().is('br'))
                $(el).children().last().children().last().remove();
            if(el.childNodes.length){
              var data;
              if(el.childNodes[el.childNodes.length-1].nodeType === 3){
                data = 'data';
              }
              else{
                data = 'innerHTML';
              }
              if(el.childNodes[el.childNodes.length-1][data])
                el.childNodes[el.childNodes.length-1][data] = el.childNodes[el.childNodes.length-1][data] + content;
              else
                el.childNodes[el.childNodes.length-1][data] = content;
            }
            else
                $(el).append(content);
          if(el.childNodes[el.childNodes.length-1].nodeType === 3){
            anchor = el.childNodes[el.childNodes.length-1];
          }
          else{
            anchor = el.childNodes[el.childNodes.length-1].childNodes[0];
          }
        }
        if(!anchor){
            anchor = el.childNodes[el.childNodes.length-1];
        ...
        if(!offset){
            offset = el.childNodes[el.childNodes.length-1][el.childNodes[el.childNodes.length-1].nodeType === 3 ? 'data' : 'innerHTML'].length-2+content.length;
        }
        // .focus()
        // .trigger(this.o.inputEvent);
...
EmojiArea.DEFAULTS = {
  ...
  inputSelector: 'input:text, textarea, div[contenteditable]',

(function ($) {
  ...
  function Paint(el, options) {
    ...
    if(this.width > this.height){
      this.heightHtml = this.height;
      this.widthHtml = this.width;
      this.width  = 1200*(this.width / this.height);
      this.height = 1200;
    }
    else{
      this.widthHtml = this.width;
      this.heightHtml = this.height;
      this.height = 1200*(this.height / this.width);
      this.width  = 1200;
    }
  ...
  Paint.prototype = {
    generate: function () {
      ...
      function createCanvas(name) {
        ...
        // _this[ctxName].drawImage($('#tempImg')[0] , 0 , 0 , _this.width , _this.height);
        ...
        _this['$' + canvasName]
            ...
            if(name != 'temp')
              _this['$' + canvasName]
            .css({height:_this.heightHtml,width:_this.widthHtml});
    ...
    resize: function () {
      ...
      this.width = this.$el3.width();
      this.height = this.$el3.height();

      if(this.factorScale == 1){
        $('#wPaint3').css('overflow' , 'hidden');
      }
      else{
        $('#wPaint3').css('overflow' , 'auto');
      }

      this.widthHtml = this.firstWidth*this.factorScale;
      this.heightHtml = this.firstHeight*this.factorScale;
      ...
      $(this.canvasBg).css({height: this.heightHtml, width: this.widthHtml});
      $(this.canvas).css({height: this.heightHtml, width: this.widthHtml});
      $(this.$el).css({height: this.heightHtml, width: this.widthHtml});
    ...
    getImage: function (withBg) {
      ...
      $(canvasSave)
          ...
          .css({height:this.heightHtml,width:this.widthHtml});
      // var img = document.getElementById("tempImg");
      // img.crossOrigin = 'Anonymous';
      // ctxSave.drawImage(img, 0, 0 , this.width , this.height);
      ctxSave.imageSmoothingEnabled = false;
      ...
      return canvasSave.toDataURL("image/jpeg",0.75);
    },
    ...
    _drawShapeMove: function (e, factor) {
      ...
      e.x = this.options.oldLineWidth / 2 * factor * this.factorScale+20;
      e.y = this.options.oldLineWidth / 2 * factor * this.factorScale+20;
      e.w = e.width - this.options.oldLineWidth * factor * this.factorScale;
      e.h = e.height - this.options.oldLineWidth * factor * this.factorScale;
      $(this.canvasTemp)
          .css({left: e.left-20, top: e.top-20})
          .attr('width', e.width+40)
          .attr('height', e.height+40);

      // store these for later to use in our "up" call
      this.canvasTempLeftNew = (e.left-20)*whf;
      this.canvasTempTopNew = (e.top-20)*whf;
      ...
      this.ctxTemp.lineWidth = this.options.oldLineWidth * factor * this.factorScale;
    },

    _drawShapeUp: function () {
      var whf = this.height/this.heightHtml;
      this.ctx.imageSmoothingEnabled = false;
      this.ctx.drawImage(this.canvasTemp, this.canvasTempLeftNew, this.canvasTempTopNew , this.canvasTemp.width*whf , this.canvasTemp.height*whf);
  ...
  Menu.prototype = {
    generate: function () {
      this.$menu = $('<div class="wPaint-menu wPaint-menu-style"></div>');
      ...
      // append menu
      this.wPaint.$el.closest('#wPaint2').append(this.$menu);
    ...
    reset: function () {
      ...
      for (key in menu.items) {
        if(key == 'diamond' || key == 'loadFg' || key == 'loadBg' || key == 'bucket' || key == 'rectangle' || key == 'circle'  || key == 'roundedRect' || key == 'square' || key == 'pentagon' || key == 'hexagon' || key == 'roundedSquare')
          continue;
    ...
    _createIconBase: function (item) {
      ...
      if ($.isNumeric(item.index)) {
        $iconImg
            .css({
              backgroundImage: 'url(/backend/web/nadiya_assets/plugins/wPaint-2.5.0/wPaint-2.5.0/' + item.img + ')',

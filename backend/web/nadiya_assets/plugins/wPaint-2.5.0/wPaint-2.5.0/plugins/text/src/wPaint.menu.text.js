(function ($) {
  ...
  $.fn.wPaint.menus.text = {
    ...
    items: {
      ...
      fontSize: {
        ...
        value: 12,
        callback: function (size) { this.setFontSize(size); this.oldFontSize = size; }
      },
      fontFamily: {
        icon: 'select',
        title: 'Font Family',
        range: ['ERPX', 'Courier', 'Times', 'Verdana'],
        useRange: true,
        value: 'ERPX',
  ...
  $.extend($.fn.wPaint.defaults, {
    ...
    fontFamily     : 'ERPX', // active font family for text input
  ...
  $.fn.wPaint.extend({
    ...
    setFontSize: function (size) {
      var _size = size*this.factorScale;
      this.options.fontSize = parseInt(_size, 10);
      this._setFont({fontSize: _size + 'px', lineHeight: (_size*1.2) + 'px'});
    ...
    _drawText: function () {
      ...
      fontString += this.options.fontSize*whf + 'px ' + this.options.fontFamily;
      ...
      for (i = 0, ii = lines.length; i < ii; i++) {
        ...
        width = textInputWidth - width + 1;
        this.ctx.fillStyle = this.options.fillStyle;
        this.ctx.textBaseline = 'top';
        this.ctx.font = fontString;
        this.ctx.fillText(lines[i], (left+width)*whf, (top+0.7)*whf);

        top += this.options.fontSize * 1.2;

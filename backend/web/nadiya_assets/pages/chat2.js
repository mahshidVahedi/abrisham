...
!function($) {
    ...
    ChatApp.prototype.contactSearcherCreate = function() {
            ...
            $.ajax({
                ...
                success:function (value) {
                    ...
                    if(value){
                        ...
                        contact += '</select></div>';
                        var html = contact + orgs.join('') + bases.join('');
                        if(statusS)
                            html += statusS.join('');
                        $academic.parent().nextAll().remove();
        ...
        $('body').on('change' , '.contact_type' , function(){
            ...
            $start.find('.organization_contact , .base_contact , .status_contact').each(function(){
                $(this).parent().hide();
            });
        ...
        $('body').on('change' , '.organization_contact' , function(){
            var $start = $(this).closest('.contact-start');
            $start.find('.base_contact , .status_contact').each(function(){
                $(this).parent().hide();
            });
        ...
        $('body').on('change' , '.base_contact' , function(){
            var $start = $(this).closest('.contact-start');
            $start.find('.status_contact').each(function(){
                $(this).parent().hide();
            });
        ...
        $('body').on('change' , '.contact-start select:visible' , function(){
                    ...
                    for(var item of value.data){
                        ...
                        if(!dateTime)
                            ...
                            if($this.$chatId != item.id){
                                ...
                                notifyMe({'text':$this.getRealText(item.last_message.text , 'text' , 'text'),'tag':'new_message_'+item.id} , item.title , item.src);
                            }
                        }
                    }
                    var $replytitle = '';
        ...
        if(mode == 'online'){
            ...
            if(!message.is_me)
                if(document.visibilityState == 'hidden')
                    notifyMe({'text':$this.getRealText(message.message_text , 'text' , 'text'),'tag':'new_message_'+$this.$chatId} , message.full_name , message.avatar_dir , $firstImage);
        }
        index++;
        if(messages[index])
        ...
        $('#vr-btn').on('click', function (ev) {
            ...
            if($this.hasClass('vr-start')){
                ...
                swal({
                    ...
                    if(!confirm)
                return false;
                recorderObject.exportBlob(function(blob) {

                    var file = new File([blob], 'voice-recording.mp3', {type:'mp3'});

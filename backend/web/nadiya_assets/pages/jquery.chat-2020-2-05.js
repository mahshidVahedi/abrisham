...
function chatTimeDiff(time) {
    ...
    if(year){
        if(year == 1)
            ...
            seenTitle = numbersToPersian(year) + ' سال پیش';
    }
    else if(month){
...
!function($) {
            ...
            $this.$searchAjax = $.ajax({
                ...
                success : function (value) {
                    ...
                    if(value){
                        ...
                        if(!value.global.length && !value.message.length) {
                            if(!offset){
    ...
    ChatApp.prototype.download = function(el , type = 'image') {
        ...
        if(type == 'image'){
            ...
            var fileId = $(el).find('img').data('file-id');
            if (fileId) {
                xhttp.open("GET", '/backend/web/users/getfile2?fileId='+fileId, true);
    ...
    ChatApp.prototype.getRealText = function(content , type = 'text' , mode = 'content' , isHtmlSC = false) {
        var textArr = [];
        if(mode == 'text')
    ...
    ChatApp.prototype.save = function(chatText = '' , filesId = '' , randId = 0) {
        var $this = this;
        if(!chatText && !filesId)
            chatText = $this.getRealText($this.$chatInput);
                    ...
                    if(value){
                        ...
                        if(value.error){
                            ...
                            if (edit){
                                ...
                                $('.chat-message-box[data-message-id="' + edit + '"] p').html(value.message_text);
                                $this.resize();
                            }
                            else{
        ...
        ChatApp.prototype.contactInsert = function(user , type = '') {
            if(!type)
                type = user.type;
            var diff = chatTimeDiff(user.activity_date);
            // if(!$.inArray(user.user_id, this.$usersPush))
                this.$usersPush.push(user.user_id);
            $('.conversation-manage-body > .conversation-list-ul .contact-start').after(
                '<li class="list-group-item conversation-list-item conversation-start-chat" data-search-type="'+type+'" data-user-id="'+user.user_id+'">' +
        ...
        ChatApp.prototype.memberInsert = function(member , value) {
            ...
            var menu = '';
            var adminIcon = '';
            if(member.user_id != $this.$userInfo.userId && value.access.is_admin){
                menu =  '<div class="message-option-menu">' +
                    '<ul class="list-group" style="text-align: center;margin: 0;">';
                if(value.access.is_owner){
                    if(!member.is_admin)
                        menu += '<li class="list-group-item add-admin-conversation"><i class="fa fa-user"></i> ادمین</li>';
                    else
                        menu += '<li class="list-group-item remove-admin-conversation"><i class="fa fa-trash"></i> حذف ادمین</li>';
                }
                if(value.access.is_admin){
                    if(!member.is_admin || value.access.is_owner)
            ...
            'success' : function (value) {
                if(value){
                    ...
                    if(!value.error){
                        ...
                        if(type == 'add'){
                            el.find('.add-admin-conversation').addClass('remove-admin-conversation').html('<i class="fa fa-user"></i> حذف ادمین').removeClass('add-admin-conversation');
    ...
    ChatApp.prototype.createChatList = function(item , type = 'chatList') {
        ...
        var messageDate;
            ...
            list = '<li class="list-group-item conversation-list-item" data-id="'+item.id+'" data-search-type="'+item.chat_type+'" data-user-id="'+item.user_id+'" data-message-id="'+item.last_message.id+'">' +
            ...
            if(item.last_message.id){
                if(item.chat_type == 2 && type != 'globalSearchResult')
                ...
                if(item.last_message.hasFile){
                    if(item.last_message.hasFile.type == 0){
                        ...
                        if(item.last_message.text){
                            ...
                            item.last_message.text = '<i class="fa fa-music"></i> موزیک';
                        }
                    }
                }
            ...
            'success' : function (value) {
                ...
                if(value) {
                    ...
                    for(var count of value.count){
                        ...
                        $this.$conversationListUl.find('.conversation-list-item[data-id="'+count.id+'"] .chat-new-message-count').html(count.count);
                    }
                }
                imgWhenLoadShow('.conversation-list-item img');
                if(!dateTime)
    ...
    if(!isHasTemp){
                ...
                if (message.file.image.length) {
                    ...
                    for (var $id in $styleDiv) {
                        ...
                        imageGallery+=$float2LineSetStart + '<div class="message-img ' + $styleDiv[$id]['h'] + $styleDiv[$id]['w'] + '">' +
                            imgIcon +
                            '<img style="' + $style + '" class=" ' + status + '" data-file-size="' + image.size + '" data-file-id="' + image.id + '" src="data:' + image.type + ';base64,' + image.srcThumb + '" data-src="' + image.src + '" data-name="'+image.name+'">' +
            ...
            if (message.file.audio) {
                if (message.file.audio.length) {
                    ...
                    for(var attach of message.file.audio){
                        attachAudio +=
                            '<div class="message-attach-audio" data-file-size="'+attach.size+'" data-file-id="'+attach.id+'">' +
                            '   <audio class="audio-waite-to-proccess" controls preload="metadata" download-url="/backend/web/users/getfile2?fileId='+attach.id+'&download=true">' +
                            '       <source src="/backend/web/users/getfile3?fileId='+attach.id+'" type="audio/mpeg">' +
                            '   </audio>' +
                            '</div>';
                    }
                }
            ...
            if (message.file.attach) {
                if (message.file.attach.length) {
                    ...
                    for(var attach of message.file.attach){
                        ...
                        attachFiles +=
                            '<div class="message-attach-file" data-file-size="'+attach.size+'" data-file-id="'+attach.id+'">' +
        ...
        var messageData = $('<li class="clearfix chat-message-box '+odd+$updateClass+$messageStatus+$isNextGallery+hideWait+'" '+$tempId+' data-message-id="' + message.message_id +'">'+
            ...
            '<div class="message-status"><span></span><div class="message-loading" style="position: relative; margin-top: -4px"><svg class="fa-spin svg-loader mini"><circle class="circle-loader"></circle></svg><div></div></div></div>'+
...
`;
    ...
    if(mode == 'online'){
        ...
        if(!message.is_me)
            if(document.visibilityState == 'hidden')
                notifyMe({'text':$this.getRealText(message.message_text , 'text' , 'text'),'id':'new_message_'+$this.$chatId} , message.full_name , message.avatar_dir , $firstImage);
    }
    index++;
    if(messages[index])
    ...
    ChatApp.prototype.init = function (type = 'withEvent') {
        ...
        $this.$chatInput.off().keypress(function (ev) {
            var p = ev.which;
            if (p == 13 && $this.shiftPress) {
                ev.preventDefault();
        ...
        $this.$chatInput.on('keyup change paste' , function (ev) {
            if($(this).text().trim())
        ...
        $this.$chatSendBtn.off('click').click(function (ev) {
            var randId = (Math.floor(Math.random() * 100000000)).toString();
            var text = $this.getRealText($this.$chatInput , 'text' , 'content' , true);
            var textForSave = $this.getRealText($this.$chatInput);
        ...
        //binding send button click
        $this.$chatList.off('click').on('click' , '.reply' , function () {
        ...
        $this.$scrolToEndBtn.off().click(function () {
            var scrollSize = $this.$chatList[0].scrollHeight;
            var scrolled = $this.$chatList[0].scrollTop;
            var height = $this.$chatList.outerHeight();
        ...
        $this.$chatList.off('scroll').on('scroll', function (e) {
        ...
        $this.$chatList.on('click' , '.message-gallery .download-img-div .download-icon' , function (el) {
        ...
        $this.$chatGallery.off('click').on('click' , '.close-show-gallery' , function (el) {
            // window.location.hash = '#'+$this.$chatId;
        });

        $this.$attachPreview.off('click').on('click' , '.close-show-attachs' , function (el) {
            // window.location.hash = '#'+$this.$chatId;
        });
        ...
        $this.$attachBtn.off('click').on('click', function (el) {
            var sec = 0;
        ...
        $this.$sendAttachBtn.off('click').on('click', function (el) {
            var images=[];
            var attach=[];
            var video=[];
            var i=2
            var fileUploadId=[];
            var messageText = $this.getRealText($this.$sendAttachText , 'text' , 'content' , true);
            var messageTextForSave = $this.getRealText($this.$sendAttachText);
            ...
            if(attach.length){
                var index=0;
                var isFile = $this.$attachInput.data('type') == 'file' ? 1 :( $this.$attachInput.data('type') == 'audio' ? 2 : ( $this.$attachInput.data('type') == 'video' ? 3 : 4));
                        ...
                        if(isFile == 2){
                            const player = Array.from(
                                $('.message-attach-file[data-file-id="' + value + '"]')
                                    ...
                                    .html(
                                        '   <audio class="audio-waite-to-proccess" controls preload="metadata" download-url="/backend/web/users/getfile2?fileId='+value+'&download=true">' +
                                        '       <source src="/backend/web/users/getfile3?fileId='+value+'" type="audio/mpeg">' +
                                        '   </audio>'
            ...
            if(video.length){
                ...
                $.ajax({
                    ...
                    success: function (value) {
                        ...
                        $('.message-attach-video').attr('data-file-id' , value);
                        index++;
                        if (!images[index]) {
                            $this.save(messageTextForSave , fileIds, randId);
                            return false;
        ...
        $this.$attachDiv.children().off('click').on('click', function (el) {
            if($(this).hasClass('image-attach')){
                $this.$attachInput.data('type','image').attr('accept','image/jpg,image/jpeg,image/png,image/gif');
            }
        ...
        $this.$attachInput.off('click').on('change', function (e) {
            window.location.hash = '#'+$this.$chatId+'?attach-file';
        });

        $this.$chatInput.off('paste').on('paste', function (el) {
            var thisEl = this;
            setTimeout(function () {
                $(thisEl).html($this.getRealText($(thisEl) , 'html' , 'content' , true));
            } , 1);
        });

        $this.$sendAttachText.off('paste').on('paste', function (el) {
            var thisEl = this;
            setTimeout(function () {
                $(thisEl).html($this.getRealText($(thisEl) , 'html' , 'content' , true));
            } , 1);
        ...
        $(window).on('hashchange' , function () {
            ...
            if(result['action'] === 'show-gallery' && result['data']['data-file-id']){
                ...
                $this.attachFilePreviwe($this.$attachInput[0] , $this.$attachInput.data('type'));
            }
        ...
        $(document).ready(function () {
            if($('.conversation-chat-box').length) {
                ...
                $(document).on('click' , '.message-video-download' , function () {
                    ...
                    var url = '/backend/web/users/getfile2?fileId='+id+'&download=true';
                    window.open(
                        url,
                ...
                $(document).on('change' , '#add-chat-avatar-input , #update-add-chat-avatar-input' , function () {
                    ...
                    $('#upload-chat-avatar').one('click' , function () {
                        $uploadCrop.result("base64" , 'viewport' , 'png' , 0.5).then(function(file) {
                ...
                $(document).on('click' , '#conversation-profile-add-member' , function () {
                    ...
                    console.log(clone.find('.conversation-list-ul'));
                    clone.append('<a class="btn circle-btn content-middle fadeOut-zoom" id="conversation-profile-add-member-check" style=" position: absolute; bottom: 10px; left: 5px; background-color: #2da6ba; color:white; padding: 14px"><i class="fa fa-check"></i><i class="fa fa-circle-o-notch fa-spin"></i></a>');
                ...
                $(document).on('click' , '.forward-chat-message' , function () {
                    ...
                    clone.find('.create-conversation-form').remove();
                    clone.find('.create-conversation-title').remove();
                    clone.find('.conversation-list-ul').css('overflow-y' , 'auto').css('height' , '100%');
                ...
                $(document).on('click' , '#conversation-profile-edit-info' , function () {
                    ...
                    <div class="create-conversation slide-reverse-right p-10">
                        <div class="update-conversation-form clearfix">
                            ...
                            <a class="btn circle-btn content-middle update-ch-g" id="conversation-profile-add-member-check" style=" position: absolute; bottom: 10px; left: 5px; background-color: #2da6ba; color:white; padding: 14px">
                ...
                $(document).on('click' , '.message-attach-video .message-video-play' , function () {
                    ...
                    $(this).html(`<svg class="fa-spin svg-loader"><circle class="circle-loader"></circle></svg>`);
                    thisVideoDiv.find('video').remove();
                    thisVideoDiv.append(`
                        <video class="video-message-chat" autoplay controls style="margin-bottom:-5px; padding:0px !important;display: none; `+style+`" download-url="/backend/web/users/getfile2?fileId=`+id+`">
                            <source src="/backend/web/users/getfile3?fileId=`+id+`">
                        </video>
                ...
                $(document).on('click' , '.submit-ch-g:not(.loading) , .update-ch-g:not(.loading)' , function () {
                    var $form;
                    var $randColor;
                    var $chatId;
                ...
                $('#chat-exit').click(function () {
                    swal({
                        ...
                        .then((confirm) => {
                        if (confirm) {
                            window.location.href = '/backend/web/users/panelnew';
                        }
                    });
                });
